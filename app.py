...
@st.cache_data
def load_data():
    # 1. Define Timeframe (10 Years for deep context)
    end_date = datetime.datetime.now()
    start_date = end_date - datetime.timedelta(days=365*10)

    # 2. Define Series (Includes Target Lower)
    series = {
    ...
    # FORCE FILL: This prevents 'Target Lower' from vanishing if there's a gap
    df = df.ffill()
...
with st.sidebar:
    ...
    # The Full List (Includes Target Lower)
...
# Logic: Ensure date range is valid
if len(dates) != 2:
    st.stop()
...
# --- ROW 1: RATES (With Corridor Fix) ---
# 1. Background Reference Lines (The Corridor)
...
# 2. The Selected Trading Pair
fig.add_trace(go.Scatter(x=data.index, y=data[long_leg], name=f"Long: {long_leg}", 
                         line=dict(color='#00CC96', width=2)), row=1, col=1)
fig.add_trace(go.Scatter(x=data.index, y=data[short_leg], name=f"Short: {short_leg}", 
                         line=dict(color='#EF553B', width=2, dash='dot')), row=1, col=1)

# --- ROW 2: SPREAD (Signal) ---
fig.add_trace(go.Scatter(x=data.index, y=spread_vals.clip(lower=0), name="Profit", 
                         fill='tozeroy', line=dict(color='green', width=0), opacity=0.5, showlegend=False), row=2, col=1)
fig.add_trace(go.Scatter(x=data.index, y=spread_vals.clip(upper=0), name="Loss", 
                         fill='tozeroy', line=dict(color='red', width=0), opacity=0.5, showlegend=False), row=2, col=1)
fig.add_trace(go.Scatter(x=data.index, y=spread_vals, name="Spread", 
...
# --- LAYOUT ---
fig.update_layout(height=900, title_text="Institutional Market Dashboard", hovermode="x unified",
                  margin=dict(l=20, r=20, t=60, b=20),
                  legend=dict(orientation="h", y=1.01, xanchor="right", x=1))
